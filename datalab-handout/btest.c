...
/* Weights for scoring: RATING_x_WEIGHT * correct */
#define RATING_1_WEIGHT 5
#define RATING_2_WEIGHT 4
#define RATING_3_WEIGHT 3
#define RATING_4_WEIGHT 1
...
static int run_tests() 
{
	...
	if (!test_fname || strcmp(test_set[i].name,test_fname) == 0) {
		...
		/* egm -- Changing the point weighting based on rating */
		if (global_rating) {
			tweight = global_rating;
		} else if (rating == 1) {
			tweight = RATING_1_WEIGHT;
		} else if (rating == 2) {
			tweight = RATING_2_WEIGHT;
		} else if (rating == 3) {
			tweight = RATING_3_WEIGHT;
		} else if (rating == 4) {
			tweight = RATING_4_WEIGHT;
		} else {
			tweight = (double)rating;
		}
		
		tpoints = tweight * tscore;
	    points += tpoints;
	    max_points += tweight;
